...
void OpenGLWindow::paintGL() {
  ...
  if(m_gameData.m_state == State::StopClick) return;

  m_gameData.m_state = State::StopClick;
  ...
  //glm::vec2 translation{m_gameData.posicaoMouse.x/300, m_gameData.posicaoMouse.y/300};
  ...
  glUniform1f(scaleLocation, 0.04f);
...
void OpenGLWindow::paintUI() {
...

  {
    ...
    if (ImGui::Button("Limpar", ImVec2(100, 30))) {
      glClear(GL_COLOR_BUFFER_BIT);
    }
...
void OpenGLWindow::handleEvent(SDL_Event &event){
  ...
  if (event.type == SDL_MOUSEBUTTONDOWN) {
    ...
    {
        m_gameData.m_state = State::StartClick;
    }
  }
